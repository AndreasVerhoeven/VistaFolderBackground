...
CPidl::CPidl(const CPidl& pidl)
{
	m_Malloc = pidl.m_Malloc;
	m_NotifyId = 0;
	m_RelativePidl = NULL;

	LPITEMIDLIST copiedPidl = ILCloneFull(pidl.m_Pidl);
	SetFromPIDL(copiedPidl);
}

CPidl& CPidl::operator=(const CPidl& pidl)
{
	ClearCache();
	FreePidl(m_Pidl);
...
bool CPidl::SetFromPath(const std::wstring& path)
{
	...
	wcscpy_s(wPath, path.length()+2, path.c_str() );
...
{
	T val = static_cast<T>(wcstol(str.c_str(),NULL,16));
...
std::wstring CPidl::StringToSHITEMID(std::wstring s, SHITEMID* itemID)
{
	...
	for(int i=0;i<cb;++i)
	{
		itemID->abID[i] = HexStringTo<BYTE>(s.substr(0,sizeof(BYTE)*2) );
